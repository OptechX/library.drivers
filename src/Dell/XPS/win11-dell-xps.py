...
win11_xps_csv = os.path.join(os.path.dirname(os.path.realpath(__file__)),'output/win11-dell-xps.csv')
win11_xps_csv2 = os.path.join(os.path.dirname(os.path.realpath(__file__)),'output/win11-dell-xps2.csv')

# remove if file exists
try:
    os.remove(win11_xps_csv)
    os.remove(win11_xps_csv2)
except OSError:
    pass

# create header row on CSV
header_row = "Make,Model,Updated"
with open(win11_xps_csv, 'a') as f:
            f.write(header_row)
            f.write('\n')
